...
parser.add_argument('--epochs_generat', default=20000, type=int, 
                    help='batch size')
...
parser.add_argument('--add_sampler',  action='store_true', 
...
for iteration_total in range(args.nb_runs):
    ...
    for iteration in range(start_iter, int(args.num_classes/args.nb_cl)):
        ...
        if iteration == start_iter:
            ...
            print(X_train.shape,np.moveaxis(X_protoset, 1, -1).shape)
            X_train    = np.concatenate((X_train,np.moveaxis(X_protoset, 1, -1)),axis=0)
        ...
        if iteration > start_iter and args.imprint_weights:
            ...
            print(iteration*args.nb_cl, (iteration+1)*args.nb_cl)
...
#         ############################################################
        ...
        if args.resume and os.path.exists(ckp_name):
            ...
            print("###############################")
            # ckp_name = './checkpoint/{}_run_{}_iteration_{}_model_next_iters.pth'.format(args.ckp_prefix, iteration_total, iteration)
        else:
            ###############################
            if iteration > start_iter and args.less_forget:
...
########################################################################################################################################################################################################################################################################################################
        ...
        if True:
            ...
            adi_data_path = './checkpoint/final_images/{}_run_{}_iteration_{}_model.pth'.format(args.ckp_prefix, iteration_total, iteration)
            # temporal data and generations will be stored here
            exp_name = './checkpoint/generations/{}_run_{}_iteration_{}_model.pth'.format(args.ckp_prefix, iteration_total, iteration)
            ...
            DeepInversionEngine = DeepInversionClass(net_teacher=tg_model,
                                                     ...
                                                     device = device,
                                                     target_classes_min = min(map_Y_train),
                                                     target_classes_max = max(map_Y_train))




            print("number of training",int((len(trainset.targets)/10)/bs))
...
#             './checkpoint/{}_run_{}_iteration_{}_class_means.pth'.format(args.ckp_prefix,iteration_total, iteration))
        ...
        evalloader = torch.utils.data.DataLoader(evalset, batch_size=eval_batch_size,
                shuffle=False, num_workers=2)
        ori_acc = compute_accuracy(tg_model, tg_feature_model, None, evalloader)
        ...
        evalloader = torch.utils.data.DataLoader(evalset, batch_size=eval_batch_size,
                shuffle=False, num_workers=2)        
        cumul_acc = compute_accuracy(tg_model, tg_feature_model, None, evalloader)
        top1_acc_list_cumul[iteration, :, iteration_total] = np.array(cumul_acc).T
        ##############################################################
        # Calculate confusion matrix
        print('Computing confusion matrix...')
        cm = compute_confusion_matrix(tg_model, tg_feature_model, None, evalloader)
