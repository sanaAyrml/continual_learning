...
print(len(X_train_total),len(X_valid_total))
...
for iteration_total in range(args.nb_runs):
    ...
    for iteration in range(start_iter, int(args.num_classes/args.nb_cl)):
        ...
        print(actual_cl)
        print(order[range(last_iter*args.nb_cl,(iteration+1)*args.nb_cl)])
        print(Y_train_total)
        ...
        print("indices_train_10",indices_train_10.shape)
        ...
        if iteration == start_iter:
            X_valid_ori = X_valid
            Y_valid_ori = Y_valid
            for i in Y_valid_ori:
                # print("here",len(Y_valid_ori))
                if i > 49:
                    print(i)
        else:
            # print(X_protoset_cumuls)
        ...
        print(last_iter*args.nb_cl, (iteration+1)*args.nb_cl)
        for iter_dico in range(last_iter*args.nb_cl, (iteration+1)*args.nb_cl):
            ...
            # print(D.shape,np.linalg.norm(D,axis=0).shape)
            ...
            iter_herding     = 0
            iter_herding_eff = 0
            # print(np.sum(alpha_dr_herding[index1,:,index2]!=0))
